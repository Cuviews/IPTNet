...
from .embeddingTOdetr import EmbeddingToDETRInput
from .fusionwithwarp import DenseMatcher,SpatialTransformer
from .Deepfake import LFGA, CMCE
from .TFFandSFF import SFF,TFF
from .FFM_in_transformer import FRM
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8,
        ...
        if deformable_decoder:
            ...
            # final_decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
            #                                                         dropout, activation,
        ...
        # final_decoder_norm = nn.LayerNorm(d_model)
        ...
        # self.final_decoder = TransformerDecoder(final_decoder_layer, num_decoder_layers, final_decoder_norm,
        #                                   return_intermediate=return_intermediate_dec,
        #                                   d_model=d_model, query_dim=query_dim,
        #                                   modulate_hw_attn=modulate_hw_attn,
        #                                   num_feature_levels=num_feature_levels,
        ...
        # self.fusionwithwarp = DenseMatcher()
        # self.spatialtransformer = SpatialTransformer(75,100,True)
        self.CMCE = CMCE(256)
        self.LFGA = LFGA(in_channel=256, ratio=4)
        # self.SFF = SFF(256)
        # self.TFF = TFF(256,256)
        # self.FRM = FRM(256)
        # self.fusion_layer = nn.MultiheadAttention(d_model, num_heads=8)
        # decoder_layeranother = DeformableTransformerDecoderLayeranother(d_model, dim_feedforward,
        #                                                   dropout, activation,
        #                                                   num_feature_levels, nhead, dec_n_points)
        ...
        # self.temporal_decoder = TemporalDeformableTransformerDecoderanother(decoder_layeranother, num_layers=6, return_intermediate=False)
    ...
    def forward(self, srcs, masks, srcs_t, masks_t, refpoint_embed, pos_embeds, pos_embeds_t, tgt, attn_mask=None, class_embed=None, embeddings1=None, embeddings2=None):
        ...
        def visualize_tensor(tensor,path):
            # 确保张量是浮点类型的
            ...
            for i in range(tensor.size(0)):
                ...
                for j in range(tensor.size(1)):
                    # 取出一个通道的数据
                    channel_data = tensor[i, j, :, :]
        
                    # 将通道数据转换为numpy数组，并调整数据范围为[0, 1]
                    channel_data = (channel_data + 1) / 2.0
        
                    # 使用matplotlib展示图像
                    plt.imshow(channel_data.numpy(), cmap='viridis')
                    plt.axis('off')  # 不显示坐标轴
                    plt.show()
                    plt.savefig(path)
                    plt.close()  # 关闭图像，释放资源
        ...
        # 调用可视化函数
        #visualize_tensor(srcs[0],'/mnt/disk1/guochuanwang/postdino/DINOvideo/results3/heatmap1.jpg')
        

        # srcs[0] = self.TFF(srcs[0], srcs_t[0])
        # srcs[1] = self.TFF(srcs[1], srcs_t[1])
        # srcs[2] = self.TFF(srcs[2], srcs_t[2])
        # srcs[3] = self.TFF(srcs[3], srcs_t[3])

        # srcs[0] = self.SFF(srcs_t[0],srcs[0])
        # srcs[1] = self.SFF(srcs_t[1],srcs[1])
        # srcs[2] = self.SFF(srcs_t[2],srcs[2])
        # srcs[3] = self.SFF(srcs_t[3],srcs[3])

        # srcs[0] = self.SFF(srcs[0], srcs_t[0])
        # srcs[1] = self.SFF(srcs[1], srcs_t[1])
        # srcs[2] = self.SFF(srcs[2], srcs_t[2])
        # srcs[3] = self.SFF(srcs[3], srcs_t[3])
        srcs[0], srcs_t[0] = self.CMCE(srcs[0], srcs_t[0])
        srcs[1], srcs_t[1] = self.CMCE(srcs[1], srcs_t[1])
        srcs[2], srcs_t[2] = self.CMCE(srcs[2], srcs_t[2])
        srcs[3], srcs_t[3] = self.CMCE(srcs[3], srcs_t[3])

        srcs[0] = self.LFGA(srcs[0], srcs_t[0])
        srcs[1] = self.LFGA(srcs[1], srcs_t[1])
        srcs[2] = self.LFGA(srcs[2], srcs_t[2])
        srcs[3] = self.LFGA(srcs[3], srcs_t[3])
        
        # srcs[0] = self.FRM(srcs[0], srcs_t[0])
        # srcs[1] = self.FRM(srcs[1], srcs_t[1])
        # srcs[2] = self.FRM(srcs[2], srcs_t[2])
        # srcs[3] = self.FRM(srcs[3], srcs_t[3])
...
#        #从这里开始是为了bind拼在后面
#        src_flatten.append(embeddings1)
#        src_flatten.append(embeddings2)
#
#        # 同样处理 mask 和位置编码（假设新 embedding 不需要 mask 或使用全 0 的 mask）
#        new_mask = torch.zeros(embeddings1.shape[:2], dtype=torch.bool).to(srcs[0].device)  # 形状 [1, 256]
#        new_mask2 = torch.zeros(embeddings2.shape[:2], dtype=torch.bool).to(srcs[0].device)
#        mask_flatten.append(new_mask)
#        mask_flatten.append(new_mask2)
#
#        # 位置编码也需要相应处理，假设我们使用全零位置编码
#        new_pos_embed = torch.zeros_like(embeddings1).to(srcs[0].device)  # 形状 [1, 256, d_model]
#        new_pos_embed2 = torch.zeros_like(embeddings2).to(srcs[0].device)
#        lvl_pos_embed_flatten.append(new_pos_embed)
#        lvl_pos_embed_flatten.append(new_pos_embed2)
#
#        new_spatial_shape = (16,16)
#        spatial_shapes.append(new_spatial_shape)
#
#        new_original_mask= torch.zeros((bs,16,16),dtype=torch.bool).to(srcs[0].device)
#        masks.append(new_original_mask)
#        print(masks)
#        #到这里结束
        ...
        # two stage
        enc_topk_proposals = enc_refpoint_embed = None  # 初始化 encoder前topk个内容 对应的参考点
        ...
        if self.two_stage_type == 'standard':
            ...
            output_memory, output_proposals = gen_encoder_output_proposals(
                memory[0:1], mask_flatten[0:1], spatial_shapes, input_hw)
            output_memory = self.enc_output_norm(
                self.enc_output(output_memory))
            bs, _, c = output_memory.shape
        ...
        hs, references = self.decoder(
            ...
            memory=memory[0:1].transpose(0, 1),
            memory_key_padding_mask=mask_flatten[0:1],
            pos=lvl_pos_embed_flatten[0:1].transpose(0, 1),
            refpoints_unsigmoid=refpoint_embed.transpose(0, 1),
            level_start_index=level_start_index,
            spatial_shapes=spatial_shapes,
            valid_ratios=valid_ratios[0:1], tgt_mask=attn_mask)
        #########################################################
        # End Decoder
        ...
        # #ref_spatial_shapes = spatial_shapes.expand(self.num_ref_frames, 4,2).contiguous()
        ...
        # #frame_start_index = torch.cat((ref_spatial_shapes.new_zeros((1, 4)), ref_spatial_shapes.prod(2).cumsum(0)[:-1, :]),dim=0).contiguous()
        #
        # #valid_ratios = valid_ratios[0:1].expand(self.num_ref_frames,4 , 2)
        # #reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=cur_memory.device)
        #
        # # if self.TDAM:  # TDTE
        # #     cur_memory = self.temporal_encoder_layer(cur_memory, cur_pos_embed, reference_points, ref_memory,
        # #                                                     ref_spatial_shapes, frame_start_index)
        #
        # # inter_references_out = references
        # # last_hs = hs[-1]
        ...
        # #
        # # topk_values, topk_indexes = torch.topk(prob.view(ref_hs_logits.shape[0], -1), 80 * self.num_ref_frames,
        # #                                        dim=1)
        ...
        # #
        # # topk_values, topk_indexes = torch.topk(prob.view(ref_hs_logits.shape[0], -1), 50 * self.num_ref_frames,
        # #                                        dim=1)
        # # topk_indexes = topk_indexes // ref_hs_logits.shape[2]
        ...
        # #
        # # topk_values, topk_indexes = torch.topk(prob.view(ref_hs_logits.shape[0], -1), 30 * self.num_ref_frames,
        # #                                        dim=1)
        # # topk_indexes = topk_indexes // ref_hs_logits.shape[2]
        ...
        #
        #
        # # final_hs, final_references_out = self.final_decoder(
        # #             tgt=cur_hs.transpose(0, 1),
        # #             memory=cur_memory.transpose(0, 1),
        # #             memory_key_padding_mask=mask_flatten[0:1],
        # #             pos=cur_pos_embed.transpose(0, 1),
        # #             refpoints_unsigmoid=cur_reference_out.transpose(0, 1),
        # #             level_start_index=level_start_index,
        # #             spatial_shapes=spatial_shapes,
        # #             valid_ratios=valid_ratios[0:1])
        ...
        # return final_hs, final_references_out ,hs_enc,ref_enc,init_box_proposal
        # hs: (n_dec, bs, nq, d_model)
...
class TemporalQueryEncoderLayer(nn.Module):
    ...
    def forward(self, query, ref_query, query_pos=None, ref_query_pos=None):
        ...
        tgt2 = self.self_attn(q.transpose(0, 1), k.transpose(
            0, 1), query.transpose(0, 1))[0].transpose(0, 1)
...
class DeformableTransformerDecoderLayeranother(nn.Module):
...
class TemporalDeformableTransformerDecoderanother(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
                ...
                if self.dec_layer_number is not None and layer_id != self.num_layers - 1:
                    ...
                    if nq_now != select_number:
                        class_unselected = self.class_embed[layer_id](
                            output)  # nq, bs, 91
                        topk_proposals = torch.topk(
                            class_unselected.max(-1)[0], select_number, dim=0)[1]  # new_nq, bs
                        new_reference_points = torch.gather(
                            new_reference_points, 0, topk_proposals.unsqueeze(-1).repeat(1, 1, 4))  # unsigmoid
            ...
            if self.dec_layer_number is not None and layer_id != self.num_layers - 1:
                if nq_now != select_number:
                    output = torch.gather(
                        output, 0, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model))  # unsigmoid
