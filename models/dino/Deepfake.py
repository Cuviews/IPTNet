...
class CMCE(nn.Module):  # Contrastive Multimodal Contrastive Enhancement  
...
class LFGA(nn.Module):  # Local Feature Guidance Attention 旨在引导特征图的注意力以更好地聚焦在局部特征上
    ...
    def forward(self, fa, fb):
        ...
        # visualize_attention_map(energy.view(B,H,W,H,W))
...
def visualize_attention_map(attention_map, save_path='attention_map.png'):
    attention_map = attention_map.detach().cpu().numpy()
    B, H1, W1, H2, W2 = attention_map.shape
    # H = W = int(np.sqrt(HW1))  # Assuming the attention map is square

    fig, axes = plt.subplots(B, 1, figsize=(100, 100))
    if B == 1:
        axes = [axes]

    for i in range(B):
        ax = axes[i]
        ax.imshow(attention_map[i].reshape(
            H1, W1, H2, W2).mean(axis=(0, 2)), cmap='viridis')
        ax.set_title(f'Attention Map for Sample {i}')
        ax.axis('off')
