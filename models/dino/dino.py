...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        if num_feature_levels > 1:
            ...
            self.input_proj_t = nn.ModuleList(input_proj_list)
        else:
...
#        self.transformer.final_decoder.class_embed = self.class_embed2
    ...
    def forward(self, samples: NestedTensor, samples2: NestedTensor,targets:List=None):
        ...
        features, poss = self.backbone(samples)  #nested tensor 包含mask 经过 backbone 得到多尺度的特征 和 层间 位置编码 其中feature包含 mask
        features2, poss2 = self.backbone(samples2)
        ...
        if self.num_feature_levels > len(srcs):             #这一部分是为了把额外的第4层小尺度的特征得到  同时得到附属的mask pos
            _len_srcs = len(srcs)
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal = self.transformer(srcs, masks,srcs_t,masks_t,input_query_bbox, poss,poss2,input_query_label,attn_mask,self.class_embed[-1])
