...
class DenseMatcher(nn.Module):
    def __init__(self,unshare_depth=4,matcher_depth=4,num_pyramids=2):
        ...
        self.matcher1 = DispEstimator(256,matcher_depth,dilation=4)
        self.matcher2 = DispEstimator(256,matcher_depth,dilation=2)
        self.matcher3 = DispEstimator(256,matcher_depth,dilation=2)
        self.refiner = DispRefiner(256,1)
        ...
        self.CMCE = CMCE(256)
        self.LFGA = LFGA(in_channel=256, ratio=2)
          

    def match(self,feat11,feat12,feat21,feat22,feat31,feat32,feat41,feat42):
        #compute scale (w,h)
        ...
        disp3_raw = self.matcher3(feat41,feat42) 
        feat41,feat42= self.CMCE(feat42,feat41)
        feat42 = self.LFGA(feat42,feat41)
        #upsample disp and grid
        disp3 = F.interpolate(disp3_raw,[feat31.shape[2],feat31.shape[3]],mode='bilinear')
        if disp3.shape[2] != self.grid_down.shape[1] or disp3.shape[3] != self.grid_down.shape[2]:
            self.grid_down = KU.create_meshgrid(feat31.shape[2],feat31.shape[3]).cuda()

        #warp the last src(fea1) to tgt(feat2) with disp2
        feat31 = F.grid_sample(feat31,self.grid_down+disp3.permute(0,2,3,1))
        feat31,feat32= self.CMCE(feat32,feat31)
        feat32 = self.LFGA(feat32,feat31)
        ...
        disp3 = F.interpolate(disp3,[feat21.shape[2],feat21.shape[3]],mode='bilinear')
        if disp2.shape[2] != self.grid_full.shape[1] or disp2.shape[3] != self.grid_full.shape[2]:
            self.grid_full = KU.create_meshgrid(feat21.shape[2],feat21.shape[3]).cuda()

        #warp
        feat21 = F.grid_sample(feat21,self.grid_full+(disp2+disp3).permute(0,2,3,1))
        feat21,feat22= self.CMCE(feat22,feat21)
        feat22 = self.LFGA(feat22,feat21)
        ...
        disp3 = F.interpolate(disp3,[feat11.shape[2],feat11.shape[3]],mode='bilinear')
        ...
        #warp
        feat11 = F.grid_sample(feat11,self.grid_full+(disp1+disp2+disp3).permute(0,2,3,1))
        feat11,feat12= self.CMCE(feat12,feat11)
        feat12 = self.LFGA(feat12,feat11)
        #finetune
        disp_scaleup = (disp1+disp2+disp3)*self.scale
        disp = self.refiner(feat11,feat12,disp_scaleup)
        disp = KF.gaussian_blur2d(disp,(17,17),(5,5),border_type='replicate')/self.scale
        if self.training:
            return disp,disp_scaleup/self.scale,disp2,[feat11,feat21,feat31,feat41],[feat12,feat22,feat32,feat42]
        return disp,None,None,[feat11,feat21,feat31,feat41],[feat12,feat22,feat32,feat42] 
        
    def forward(self,src,tgt,type='ir2vis'):
        b0,c0,h0,w0 = tgt[0].shape
        b1,c1,h1,w1 = tgt[1].shape
        b2,c2,h2,w2 = tgt[2].shape
        b3,c3,h3,w3 = tgt[3].shape
        ...
        if type == 'bi':
            ...
            disp_12,_,_,src_t,src= self.match(feat11,feat12,feat21,feat22,feat31,feat32,feat41,feat42)
            disp_12 = F.interpolate(disp_12,[h0,w0],mode='bilinear')
        elif type =='vis2ir':
        ...
        return {'ir2vis':disp_12},src_t,src
...
if __name__ == '__main__':
    ...
    matcher = DenseMatcher().cuda().eval()
    ST=SpatialTransformer(256,256,True).cuda().eval()
    # ir = torch.rand(2,3,600,800).cuda()
    # vis = torch.rand(2,3,600,800).cuda()
    T=[torch.rand(2,256,75,100).cuda(),torch.rand(2,256,38,50).cuda(),torch.rand(2,256,19,25).cuda(),torch.rand(2,256,10,13).cuda()]
    RGB=[torch.rand(2,256,75,100).cuda(),torch.rand(2,256,38,50).cuda(),torch.rand(2,256,19,25).cuda(),torch.rand(2,256,10,13).cuda()]
    disp,T2,RGB2=matcher(T,RGB,'ir2vis')
    ir_output=ST(T2[0],disp['ir2vis'])
    print(ir_output.shape)
    # print(disp["vis2ir"].shape)
    # print(disp["down2"].shape)
    # print(disp["down4"].shape)
    # print(disp["down8"].shape)