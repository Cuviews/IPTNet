...
#                                          use_detached_boxes_dec_out=use_detached_boxes_dec_out
#                                          )
        #self.lwa = lwa(256, 256)
        self.CMCE = CMCE(256)
        self.LFGA = LFGA(in_channel=256,ratio=4)
        #self.LFGA2 = LFGA(in_channel=256,ratio=4)
        #self.CAN = ContextualModule(256,256)
        #self.CAN2 = ContextualModule(256, 256)
        #self.CAN3 = ContextualModule(256, 256)
    ...
    def forward(self, srcs, masks, srcs_t,masks_t,refpoint_embed, pos_embeds,pos_embeds_t, tgt, attn_mask=None,class_embed=None):
        ...
        #
        #             # 使用matplotlib展示图像
        #             plt.imshow(channel_data.numpy(), cmap='gray')
        #             plt.axis('off')  # 不显示坐标轴
        #             plt.show()
        #
        # # 调用可视化函数
        # visualize_tensor(srcs[0])
#      
        ...
        if self.two_stage_type =='standard':
            if self.two_stage_learn_wh:
                ...
                input_hw = None
            output_memory, output_proposals = gen_encoder_output_proposals(memory[0:1], mask_flatten[0:1], spatial_shapes, input_hw)
...
class TransformerEncoder(nn.Module):
    ...
    @staticmethod
    def get_reference_points(spatial_shapes, valid_ratios, device):   #encoder阶段的获取参考点是 整个棋盘撒
        reference_points_list = []
        for lvl, (H_, W_) in enumerate(spatial_shapes):
