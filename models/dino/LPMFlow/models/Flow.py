...
from .feature_backbones import vision_transformer
...
from .encoder.encoder.single_tower_smask import SingleTower
import random
import torch
...
class MultiscaleDecoder(nn.Module):
    def __init__(
        ...
        self.last = last
        self.gelu = nn.GELU()
        self.pos_embed_src = nn.Parameter(torch.zeros(1, 256 + 1, 1024))
        self.pos_embed_trg = nn.Parameter(torch.zeros(1, 256 + 1, 1024))
        self.pos_embed_src_up = nn.Parameter(torch.zeros(1, 1024 + 1, 1024))
        self.pos_embed_trg_up = nn.Parameter(torch.zeros(1, 1024 + 1, 1024))
...
class SRBranch(nn.Module):
    def __init__(self):
        ...
        #     TransformerWarper2d(
        #         SwinTransformer2d(
        #             img_size=(600, 800), embed_dim=96, window_size=8, num_heads=[8], depths=[1]
        #         )
        #     ),
        #     ChannelAttention(96),
        # )
        ...
        #     TransformerWarper2d(
        #         SwinTransformer2d(
        #             img_size=(600, 800), embed_dim=96, window_size=4, num_heads=[8], depths=[1]
        #         )
        #     ),
        #     ChannelAttention(96),
        # )
        ...
        #     TransformerWarper2d(
        #         SwinTransformer2d(
        #             img_size=(600, 800), embed_dim=96, window_size=2, num_heads=[8], depths=[1]
        #         )
        #     ),
        #     ChannelAttention(96),
        # )
...
class SRBranch_feat(nn.Module):
    def __init__(self):
        ...
        self.refine_swin_decoder_1_1 = nn.Sequential(
            TransformerWarper2d(
                SwinTransformer2d(
                    img_size=(32, 32), embed_dim=1024, window_size=4, num_heads=[8], depths=[1]
                )
            ),
...
class transFlow(nn.Module):
    def __init__(self, feature_size=128, ibot_ckp_file='./', depth=6, num_heads=8, mlp_ratio=4, freeze=False):
        ...
        self.masked_tokens = nn.Parameter(torch.zeros(257, 1024), requires_grad=True)
        self.pos_embed_src = nn.Parameter(torch.zeros(1, 256 + 1, 1024))
    ...
    def forward(self, src_feat, tgt_feat, is_train=False):
        Size=(600,800)
        mask_indexs = None
        ...
        flow_up_11 = F.interpolate(flow_11, Size, None, "nearest") * 4
        ...
        flow_up_12 = F.interpolate(flow_12, Size, None, "nearest") * 4
        ...
        flow_up_21 = F.interpolate(flow_21, Size, None, "nearest") * 4
        ...
        flow_up_22 = F.interpolate(flow_22, Size, None, "nearest") * 4
...
if __name__ == "__main__":
    ...
    trg = torch.rand((3, 197, 1024))
    src = torch.rand((3, 197, 1024))
    b,  L, c = trg.shape
    model = transFlow(num_heads=8)
