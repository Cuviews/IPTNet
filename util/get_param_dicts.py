...
def get_param_dict(args, model_without_ddp: nn.Module):
    ...
    assert param_dict_type in ['default', 'ddetr_in_mmdet', 'large_wd','new']
    ...
    if param_dict_type == 'new':
        param_dicts = [
            {"params": [p for n, p in model_without_ddp.named_parameters() if "backbone" not in n and "superfusion" not in n and "STN" not in n and p.requires_grad]},
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "backbone" in n and p.requires_grad],
                "lr": args.lr_backbone,
            },
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "superfusion" in n and p.requires_grad],
                "lr": args.lr_warp,
            },
            {
                "params": [p for n, p in model_without_ddp.named_parameters() if "STN" in n and p.requires_grad],
                "lr": args.lr_STN,
            }
        ]
        return param_dicts
